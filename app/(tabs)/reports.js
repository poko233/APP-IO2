...
const ReportesPedidos = () => {
  ...
  useEffect(() => {
    ...
    const showSubscription = Keyboard.addListener(keyboardDidShowListener, (event) => {
      setIsKeyboardVisible(true);
      setKeyboardHeight(event.endCoordinates.height);
    });
    
    const hideSubscription = Keyboard.addListener(keyboardDidHideListener, () => {
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return pedidos.filter(pedido => {
      ...
      if (filtroMes === 'actual') {
        ...
        return fechaPedido.getMonth() === mesSeleccionado && fechaPedido.getFullYear() === añoActual;
      }
    });
  }, [pedidos, filtroMes]);
  ...
  const estadisticasFinancieras = useMemo(() => {
    const totalVentas = pedidosFiltrados.reduce((sum, pedido) => sum + (pedido.totalAmount || 0), 0);
    const totalPedidos = pedidosFiltrados.length;
    const totalProductos = pedidosFiltrados.reduce((sum, pedido) => sum + (pedido.totalQuantity || 0), 0);
    ...
    pedidosFiltrados.forEach(pedido => {
      ...
      ventasPorMetodoPago[metodo] = (ventasPorMetodoPago[metodo] || 0) + (pedido.totalAmount || 0);
    });

    const clientesUnicos = new Set(pedidosFiltrados.map(p => p.userEmail).filter(Boolean)).size;
    ...
    setTimeout(() => {
      if (searchContainerRef.current && scrollViewRef.current) {
        searchContainerRef.current.measureInWindow((x, y, width, height) => {
          ...
          const keyboardBuffer = Platform.OS === 'android' ? keyboardHeight + 20 : 100;
  ...
  const renderMesItem = ({ item }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
    >
      <Text className={`text-base ${filtroMes === item.value ? 'text-blue-600 font-medium' : 'text-gray-700'}`}>
        {item.label}
      </Text>
    </TouchableOpacity>
  );
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      >
        ...
        >
          {/* Header */}
          <View className="mb-6">
            <Text className="text-2xl font-bold text-gray-900 mb-1">Dashboard de Reportes</Text>
            <Text className="text-gray-600">Análisis financiero y gestión de pedidos</Text>
          </View>
          ...
          <View className="bg-white rounded-lg shadow-sm p-4 mb-6">
            <View className="flex-row items-center mb-3">
              ...
              <Text className="text-sm font-medium text-gray-700 ml-2">Período de reporte:</Text>
            </View>
            <TouchableOpacity
            ...
            <View className="flex-1 justify-center items-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
              <View className="bg-white rounded-lg mx-4 max-w-sm w-full max-h-96 overflow-hidden">
                <View className="p-4 border-b border-gray-200">
                  <Text className="text-lg font-semibold text-gray-900">Seleccionar período</Text>
                </View>
          ...
          <View className="mb-6">
            ...
            <View className="bg-white p-4 rounded-lg shadow-sm mb-4">
              <Text className="text-lg font-semibold mb-3">Pedidos por Estado</Text>
              {Object.entries(estadisticasFinancieras.pedidosPorEstado).map(([estado, cantidad]) => (
                <View key={estado} className="flex-row justify-between items-center mb-2">
                  <Text className="capitalize text-gray-700">{estado}</Text>
                  <Text className="font-semibold text-gray-900">{cantidad}</Text>
                </View>
              ))}
            </View>
            ...
            <View className="bg-white p-4 rounded-lg shadow-sm mb-4">
              <Text className="text-lg font-semibold mb-3">Ventas por Método de Pago</Text>
              {Object.entries(estadisticasFinancieras.ventasPorMetodoPago).map(([metodo, total]) => (
                <View key={metodo} className="flex-row justify-between items-center mb-2">
                  <Text className="capitalize text-gray-700">
                    {metodo === 'physical' ? 'Efectivo' : 
                     metodo === 'qr' ? 'QR' : 
                     metodo}
                  </Text>
          ...
          >
            ...
            {pedidosEncontrados.length > 0 && (
              <View>
                <Text className="text-sm text-gray-600 mb-3">
                  {pedidosEncontrados.length} resultado{pedidosEncontrados.length !== 1 ? 's' : ''} encontrado{pedidosEncontrados.length !== 1 ? 's' : ''}:
                </Text>
                  ...
                  >
                    <View className="flex-row justify-between items-start mb-3">
                      <View className="flex-1">
                        ...
                        <Text className="text-sm text-gray-500">ID: {pedido.id}</Text>
                      </View>
                      <View className={`px-3 py-1 rounded-full ${getStatusColor(pedido.status)}`}>
                        <Text className={`text-xs font-medium ${getStatusTextColor(pedido.status)}`}>
                          {pedido.status || 'N/A'}
                        </Text>
                      </View>
                    </View>
                    ...
                    <View className="flex-row justify-between items-center mb-2">
                      <View className="flex-1">
                        ...
                        <Text className="font-medium text-gray-900">{pedido.userName || 'N/A'}</Text>
                      </View>
                    ...
                    <View className="flex-row justify-between items-center">
                      <View>
                        ...
                        <Text className="text-sm font-medium text-gray-900">
                          {timestampToDate(pedido.createdAt).toLocaleDateString('es-ES')}
                        </Text>
                      </View>
            ...
            {busquedaId.trim() && pedidosEncontrados.length === 0 && !buscando && (
          ...
          <View style={{ height: isKeyboardVisible ? keyboardHeight + 50 : 50 }} />
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
